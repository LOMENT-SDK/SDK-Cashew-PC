...
public class Toast extends Popup {
	...
	public void show(Node anchor, Side side, double offsetX, double offsetY) {
		...
		HPos hpos = side == Side.LEFT ? HPos.LEFT
				: side == Side.RIGHT ? HPos.RIGHT : HPos.CENTER;
		VPos vpos = side == Side.TOP ? VPos.TOP
				: side == Side.BOTTOM ? VPos.BOTTOM : VPos.CENTER;
		// translate from anchor/hpos/vpos/offsetX/offsetY into screenX/screenY
		Point2D point = Utils.pointRelativeTo(anchor, content.prefWidth(-1),
				content.prefHeight(-1), hpos, vpos, offsetX, offsetY, true);
	...
	private void tryShow() {
		if (window == null && anchor == null) {
			throw new IllegalStateException(
					"window and anchor node are both null");
		}
		if (alreadyShowing) {
			// if another toast is already showing add this one to the waiting
			// queue
	...
	private void doShow() {
		...
		if (window != null) {
			...
			if (autoCenter) {
				...
				if (window == null) {
					throw new IllegalStateException(
							"anchor node is not attached to a window");
				}
				connectAutoCenterHandler();
			}
			super.show(anchor, Double.isNaN(screenX) ? 0.0 : screenX,
					Double.isNaN(screenY) ? 0.0 : screenY);
		}
	...
	private void doHide() {
		...
		if (toastQueue.isEmpty()) {
			...
			// get the next toast and show it (may flicker if not enqueued in
			// the FX thread!?)
	...
	private static final Insets DEFAULT_CONTENT_MARGIN = new Insets(8, 16, 8,
			16);
	...
	public static Toast makeText(String text, boolean wrapText,
			double maxWidth, double maxHeight, Duration duration) {
